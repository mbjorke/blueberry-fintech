...
/**
 * Design System Validation Script for CI/CD
 * 
 * Validates all React components against design system rules:
 * - Button hierarchy
 * - Color token usage
 * - Spacing consistency
...
const { analyzeComponentUsage } = await import(
  path.join(projectRoot, "mcp-server/dist/tools/component-analyzer.js")
);
const { validateTailwindTokens } = await import(
  path.join(projectRoot, "mcp-server/dist/tools/token-validator.js")
);
  ...
  try {
    ...
    if (DESIGN_SYSTEM_RULES.buttonHierarchy) {
      try {
        ...
        console.warn(`Warning: Could not analyze component usage for ${filePath}:`, error.message);
      }
    }
    ...
    if (
      ...
      try {
        ...
        console.warn(`Warning: Could not validate tokens for ${filePath}:`, error.message);
      }
    }
  } catch (error) {
...
async function validateDesignSystem() {
  ...
  if (totalErrors > 0) {
    console.log("💡 Tip: Run 'npm run fix:design' to automatically fix common violations\n");
    process.exit(1);
  }
...
// Run validation
validateDesignSystem().catch((error) => {
