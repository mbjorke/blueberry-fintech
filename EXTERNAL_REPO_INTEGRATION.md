...
# Copy MCP server
cp -r /path/to/blueberry-fintech/mcp-server ./mcp-server

# Copy knowledge base (or reference remote)
cp -r /path/to/blueberry-fintech/mcp-knowledge-base ./mcp-knowledge-base

# Copy configuration
cp /path/to/blueberry-fintech/mcp-config.json ./
```
...
```json
// blueberry-fintech/package.json
{
  "name": "@blueberry/design-system",
  "version": "1.0.0",
...
#### 1. Design System Repo (blueberry-fintech)
...
Ready to implement? See `QUICKSTART_EXTERNAL_REPO.md` for step-by-step!
